...
export function buildStage1SystemPrompt(input: Stage1PromptInput): string {
  ...
  return [
    ...
    '',
    'CATEGORY RULES:',
    '    - category_match MUST be a LEAF category: one whose id never appears as another category\'s parent_id. Products can only belong to leaf categories.',
    '    - Choose the single most specific existing LEAF category that the product PERFECTLY fits.',
    '    - NEVER return a parent/non-leaf category id in category_match. If the best fit is a non-leaf category, set category_match to null and instead suggest a new leaf category under it.',
    '    - If a perfect leaf fit exists, return its id in category_match.',
    '    - If no existing leaf category is a good fit, set category_match to null and propose a new category in suggested_category: name_en, name_ar, and parent_id (the id of the best existing parent to place it under, or null to place it at the root). Add a short reason.',
    '',
    'STRICT OUTPUT RULES:',
